...
The current version is Lean 4, dating back to 2021. It is not backwards-compatible wih **Lean 3**.
-/
...
/- 
All three terms below are natural numbers, but Lean does not perform any kind of computation to check whether the equality in the statement is true or not. 
-/
...
Note that what all our statement have in common is that they are *equalities*. Now, in Lean, this is a strong requirement: an equality between terms in Lean means that the two terms are *definitionally* equal.

It is difficult at this moment to explain what this means, but basically it says that `1 + 1 = 2` holds because the terms `1 + 1` and `2` *reduce to the same term* (in Lean's compiler).
...
Note that, in term mode, Lean does not generates a `No goals` message. If you use a proof term, the only way to know it worked is via the absence of an error message. 
-/
...
Since by assumption we have a proof of the implication `P → Q`, it suffices to prove `P`. This is what the `apply` tactic enables us to do. More precisely, the term `f` is a function from `P` to `Q`, so it sends a term of type `P` (i.e. a proof a `P`) to a term of type `Q`, i.e. a proof of `Q`.
...
/- 
Note that in tactic mode, *one works on the goal to reduce it to the assumptions*.
...
/- 
Next, by using the command `variable`, we add to our context the Propositions `P` and `Q`, as well as a proof of the implication `P → Q` and a proof of `P`.  This is just for commodity here. 
-/
...
/- Equivalently, we can write `(P → Q) ∧ P → Q`, instead of `(P → Q) → P → Q`. The symbol `∧` is read *and* and can be obtained by typing in the command `\and` or `\we` (as in *wedge*).

In that case, the proof is slightly longer, and we wust apply the `rcases` tactic in order to replace the hypothesis `h : (P → Q) ∧ P` by the two hypotheses `f : (P → Q)` and `p : P`.

Here, the names `h`, `f` and `p` are chosen by the user. If we simply write `rcases h` instead of `rcases h with ⟨f, p⟩`, *Lean* will assign names automatically but they will not be directly usable.
-/
